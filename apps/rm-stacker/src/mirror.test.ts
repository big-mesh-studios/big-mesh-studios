...
/** That part, as the editor lays its drawings out. */
...
const marks = (_mirror: Mirror, panel: keyof Sides, x: number, y: number) =>
  mirrorMarks(_mirror, TABLE, { panel, position: { x, y } });
...
describe("mirrorMarks, reflecting within the panel", () => {
  ...
  it("covers all four quarters when both panel axes are mirrored", () => {
    expect(marks(panel(true, true), "front", 1, 0)).toEqual([
      { panel: "front", position: { x: 1, y: 0 } },
      { panel: "front", position: { x: 3, y: 0 } },
      { panel: "front", position: { x: 1, y: 4 } },
      { panel: "front", position: { x: 3, y: 4 } },
    ]);
  });

  it("stays on the panel it was drawn on", () => {
    const panels = marks(panel(true, true), "left", 0, 0).map(
      (mark) => mark.panel,
    );
    expect(new Set(panels)).toEqual(new Set(["left"]));
  });

  it("lists a mark on the panel's middle cell once", () => {
    expect(marks(panel(true, true), "front", 2, 2)).toEqual([
      { panel: "front", position: { x: 2, y: 2 } },
    ]);
  });
  ...
  it("reflects within a panel using that panel's own size", () => {
    ...
    expect(marks(panel(true, false), "left", 0, 1)).toEqual([
      { panel: "left", position: { x: 0, y: 1 } },
      { panel: "left", position: { x: 2, y: 1 } },
    ]);
  });
});
...
describe("mirrorMarks, reflecting onto the opposing panel", () => {
  ...
  it("carries a mark on the top panel to the bottom one", () => {
    expect(marks(opposing, "top", 1, 0)).toEqual([
      { panel: "top", position: { x: 1, y: 0 } },
      { panel: "bottom", position: { x: 1, y: 2 } },
    ]);
  });
  ...
  it("carrying a mark back returns it to where it began", () => {
    for (const panel of [
      ...
      const [origin, carried] = marks(opposing, panel, 0, 0);

      expect(carried).toBeDefined();
      expect(mirrorMarks(opposing, TABLE, carried)).toEqual([carried, origin]);
    }
  });
...
describe("mirrorBlocks", () => {
  ...
  it("leaves a block where it was drawn when nothing is mirrored", () => {
    expect(mirrorBlocks(NO_MIRROR, TABLE, block)).toEqual([block]);
  });

  it("swaps a block's ends when it reflects it", () => {
    expect(mirrorBlocks(panel(true, false), TABLE, block)).toEqual([
      block,
      { panel: "front", min: { x: 3, y: 0 }, max: { x: 4, y: 2 } },
    ]);
  });
  ...
  it("lists a block that already spans the panel once", () => {
    ...
    expect(mirrorBlocks(panel(true, false), TABLE, across)).toEqual([across]);
  });

  it("carries a block to the panel opposite it", () => {
    expect(mirrorBlocks(opposing, TABLE, block)).toEqual([
      block,
      { panel: "back", min: { x: 3, y: 0 }, max: { x: 4, y: 2 } },
    ]);
  });
});

// The OAuth client configuration and the popup sign-in flow, shared by the
// store in `create-atproto.ts` and the callback page. atcute's browser OAuth
// client is configured through module-level state and keeps its sessions in
...
const POPUP_CHANNEL = "rm-stacker.oauth";
 ...
 * "localhost" (RFC 8252 disallows "localhost" as a redirect_uri host), which is
 * what the redirect in `index.tsx` makes sure of.
 */
 ...
 */
export function configureOAuthClient(clientId?: string): Promise<ClientConfig> {
  configuring ??= (async () => {
 ...
 */
export async function signInPopup(params: {

...
describe("getHeightAt", () => {
  it("is deterministic for a given config", () => {
    ...
    for (const x of p) {
      for (const z of p) {
        expect(getHeightAt(x, z, DEFAULT_TERRAIN)).toBe(
          getHeightAt(x, z, DEFAULT_TERRAIN),
        );
      }
    }
  });
  ...
  it("differs between seeds", () => {
    ...
    for (let x = 0; x < 50; x++) {
      for (let z = 0; z < 50; z++) {
        if (getHeightAt(x, z, a) !== getHeightAt(x, z, b)) differing++;
      }
    }
    expect(differing).toBeGreaterThan(0);
  });
  ...
  it("stays within base ± amplitude even with plains enabled", () => {
    ...
    for (let i = 0; i < 500; i++) {
      ...
      const h = getHeightAt(x, z, DEFAULT_TERRAIN);
  ...
  it("matches pure mountains when the threshold can never be crossed", () => {
    ...
    for (let i = 0; i < 100; i++) {
      const x = i * 3.7;
      const z = i * 1.1 + 5;
      expect(getHeightAt(x, z, cfg)).toBe(getMountainHeightAt(x, z, cfg));
    }
  });
  ...
  it("averages the four surrounding plateau heights at a cell centre", () => {
    ...
    const expected =
      (getPlateauHeightAt(cx, cz, cfg) +
        getPlateauHeightAt(cx + 1, cz, cfg) +
        getPlateauHeightAt(cx, cz + 1, cfg) +
        getPlateauHeightAt(cx + 1, cz + 1, cfg)) /
      4;
    expect(getHeightAt(x, z, cfg)).toBeCloseTo(expected, 6);
  });
  ...
  it("is nearly level across the middle of an always-flat cell", () => {
    ...
    const x = (cx + 0.5) * 40;
    const z = (cz + 0.5) * 40;
    const centre = getHeightAt(x, z, cfg);
    ...
    for (let dx = -8; dx <= 8; dx += 4) {
      for (let dz = -8; dz <= 8; dz += 4) {
        const h = getHeightAt(x + dx, z + dz, cfg);
  ...
  it("flat land undulates far less than the mountains", () => {
    ...
    let flatSpread = 0;
    for (let i = 0; i < 40; i++) {
      const h = getHeightAt(x + i, z + i, cfg);
      flatSpread = Math.max(flatSpread, Math.abs(h - getHeightAt(x, z, cfg)));
    }
    let mountainSpread = 0;
    for (let i = 0; i < 40; i++) {
      const h = getMountainHeightAt(x + i, z + i, cfg);
      mountainSpread = Math.max(
        mountainSpread,
        Math.abs(h - getMountainHeightAt(x, z, cfg)),
      );
    }

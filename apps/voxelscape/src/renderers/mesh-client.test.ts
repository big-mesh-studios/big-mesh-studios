...
const setup = (
  ...
  for (let i = 0; i < blockCount; i++) {
    ...
    // These tests exercise the message protocol, not the surface sweep, so
    // mark each shell as holding surface data to keep it on the worker path.
    block.level.broadData[0] = 1;
    blocks.push(block);
  }
  const built: number[] = [];

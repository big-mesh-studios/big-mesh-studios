...
**CloudController**:
The field of blocky cloud puffs that drifts across the sky, anchored to the world rather than the camera, so a flying player can fly right through a puff, covering about 40% of the sky (tuned against `cloudCoverage`). Each puff is a static, low-poly mesh generated once from the world's terrain seed: a 3D Perlin noise (the y read scaled by `CLOUD_FLATNESS` so puffs come out flatter than they are wide) thresholded into voxels, culled-face meshed exactly like the terrain, and shaped by a coverage sample of the same periodic noise that thickens denser areas into banks. The noise is periodic over the wrap tile, which is what lets the fixed set of meshes recycle invisibly: every `tick` places each cell at `camera + wrap(anchor + drift - camera)` — the same camera-centred modulo wrap the rain and snow particles use — so the field surrounds the player on every side (a cell's symmetric anchor tiles the half-tile window completely) while each mesh keeps its world position up to a whole-tile jump, so flying through a puff still works and a camera crossing a tile boundary is seamless. `applyLighting` feeds the day-night state into the shared `CloudMaterial` — phase tint, sun/moon/ambient terms, and the weather-tinted sky colour as fog. Exposes plain typed methods (`tick`, `applyLighting`, `setVisible`, `describe`); has no idea a console exists.
_Avoid_: SkyController (that's `DayNightController`'s job), cloud renderer (there is one field of static meshes, not a rendering strategy)

**CloudMaterial**:
The material a cloud puff is drawn with: the same sun/moon/ambient lighting and distance fog as the terrain's `TriangleMaterial`, but over a flat phase tint instead of an atlas texture, so clouds warm at dusk, darken at night, and fade to the storm-tinted sky colour at distance. One shared instance across every puff.
_Avoid_: CloudShader (it's a material with shared state, not a standalone shader)
...
- **CloudController** is the third environment system with the same one-directional seam: `createEnvironment` ticks it inside `environment.tick` and feeds it the weather-mixed day-night state through `applyLighting`, and the composer places `environment.clouds.cloudField` in the scene between the sky and the terrain.

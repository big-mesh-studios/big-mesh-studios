...
Drawn from the imports under `src` at af95aa3 by `pnpm architecture`.
Nothing here is written by hand: change the code and run it again.

```mermaid
graph TD
  shell["shell<br/>6 files · 2011 lines"]
  voxelscape["voxelscape<br/>3 files · 3481 lines"]
  world["world<br/>32 files · 7906 lines"]
  renderers["renderers<br/>21 files · 6457 lines"]
  render["render<br/>4 files · 1145 lines"]
  player["player<br/>16 files · 3876 lines"]
  multiplayer["multiplayer<br/>14 files · 3663 lines"]
  places["places<br/>36 files · 16907 lines"]
...
| `voxelscape`   | one world: its frame, and every part below it                          | 3     | 3481  |
| `world`        | voxels, light, the streaming window, and the workers that fill it      | 32    | 7906  |
| `renderers`    | turning voxels into geometry, and drawing it                           | 21    | 6457  |
| `render`       | the frame loop, the resolution scaler, and the probe that times them   | 4     | 1145  |
| `player`       | the body, its input, its tools and what they do to the world           | 16    | 3876  |
| `multiplayer`  | other players, over a peer connection                                  | 14    | 3663  |
| `places`       | a published place: its script, its people, and the sandbox they run in | 36    | 16907 |
